...
#=============================반납, 대출 버튼 이벤트 =======================================
    ...
    def bookrent_selectuser():
        ...
        def event_book_rent():
                 ...
                 rentMsgBox = tkinter.messagebox.askquestion(" ",'해당 도서를 대출하시겠습니까?')
                 global num
                 if rentMsgBox == 'yes':
                    ...
                    book_df.loc[book_df['BOOK_ISBN']==select_book,'BOOK_RENTAL'] = True
                    user_df.loc[user_df['USER_PHONE']==select_user,'USER_RENT_CNT'] = 1
                    ...
                    tkinter.messagebox.showinfo("도서 대출", "도서 대출 완료")
                     ...
                     tkinter.messagebox.showinfo("취소","대출을 취소합니다.")
        ...
        booknamelabel = Label(rent1, text = ABC, bg = 'gray') # 텍스트는 선택한 도서 이름
    ...
    def event_book_return():
         ...
         if returnMsgBox == 'yes':
           select_book = BOOK_SELECT_BOX.focus()
           idx = rent_df[rent_df['BOOK_ISBN'] == select_book].index # RENT_NUM 속성에서 num번째 값을 인덱스로 저장
           rent_df.drop(index=idx,axis=1,inplace=True) # num # 인덱스로 저장한 idx를 참고하여 drop(), 해당 행 삭제
           book_df.loc[book_df['BOOK_ISBN']==select_book,'BOOK_RENTAL'] = False
           # user_df.loc[user_df['USER_PHONE']==select_user,'USER_RENT_CNT'] = 0
           rent_df.to_csv("csv/rent.csv", index = False)
           book_df.to_csv("csv/book_1.csv", index = False)
           user_df.to_csv("csv/USER1.csv", index = False)
           print(tabulate(rent_df,headers='keys',tablefmt='pretty',showindex=False,numalign='center',stralign='center'))
